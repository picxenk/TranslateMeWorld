...
- Support opening `index.html` directly without a web server. Check that Ollama is running and the model is installed, then inspect browser errors for CORS or local-network blocking.
- If direct `file://` use is blocked by CORS, explain that macOS app users can quit Ollama, run `launchctl setenv OLLAMA_ORIGINS "*"`, and reopen it. Windows users can quit Ollama, set the `OLLAMA_ORIGINS` user environment variable to `*`, and reopen it. Warn that the wildcard should only be used in a trusted local environment. When a static HTTP server is used, allow its actual page origin instead of a wildcard. Terminal-launched servers must inherit the variable.
